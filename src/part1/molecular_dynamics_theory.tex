...
\subsection{Expanding the simulation box}
Let's consider expanding the simulation box aling the x-axis. This corresponds to perform work on the system. If the process is adiabatic, then:

\begin{equation}
	W = \int_{t(L_x = L_x^0)}^{t(L_x^0 + \Delta x)} \int_{yz} \sigma_{xx} (t, y, z) \ \dd z \dd y \dd x
\end{equation}

For simulations where the particles are not allowed to have net change of velocity of angular momentum, the inner integral can be calculated from macroscopic quantities:
\begin{equation}
