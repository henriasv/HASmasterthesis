...
A considerable improvement from the rescaling approach is the thermostat of \citet{Berendsen1984}. The Berendsen thermostat introduces a timescale for the velocity rescaling by adding a temperature-dependant friction term to the equations of motion. The idea is to weakly couple the system to a heat bath by gradually rescaling velocities to satisfy:
\begin{equation}
\frac{\dd T}{dt} = \frac{T_{\text{bath}}-\mathcal{T}}{\tau}
\end{equation}
Where $T_{bath}$ is the temperature of the heat bath that is coupled to the MD-system to keep the temperature constant. 
@PutAssumptionsAndThe
