...
\caption{Crack area (top panel), crack speed (middle panel) and tensile stress (lower panel). The panels are the same as the ones in \ref{fig:area_speed_stress_all}, but they are significantly zoomed. It becomes clear from the lover panel that a higher tensile stress (from a higher strain) before rupture results in a shorter waiting time before fracture in the simulations with $T=\SI{280}{\kelvin}$ (dotted lines), bit the same systematic effect is not seen for $T=\SI{260}{\kelvin}$. Tendencies of two stages of crack growt can be seen from the upper panel. The area grows slowly until it suddenly starts to grow fast.}
