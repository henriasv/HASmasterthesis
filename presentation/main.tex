...
\foreach \i in {1, 2, ..., 100}
{
	...
	\FPeval{\resultx}{clip(\x+\xsh)}
	\FPeval{\resulty}{clip(\y+\ysh)}
	\FPeval{\resultxi}{clip(\x+1.5*\xsh+0.5*\xsi)}
	\FPeval{\resultyi}{clip(\y+1.5*\ysh+0.5*\ysi)}
	\filldraw [gray!40!white] (\x/100, \y/100) circle (1pt);
...
{
	\begin{figure}
		\includegraphics[height=0.8\textheight]{../snapshots/carved_crack.pdf}
	\end{figure}
}
\end{frame}
%\end{comment}
...
\begin{frame}
\frametitle{Simulert system for mekaniske egenskaper}
\only<1>
{
...
\begin{equation*}
 E=\frac{\sigma_x l_0}{\Delta x} = \frac{\sigma_x}{\epsilon_x}
\end{equation*}
\end{block}
...
\end{equation*}
\end{block}
Dersom den \emph{mekaniske} energien som frigjøres ved å åpne ny sprekkflate er større enn energien som kreves for å åpne sprekken, vil sprekken vokse.
\end{frame}


\begin{frame}
\frametitle{Hva var det jeg lurte på?}
\begin{itemize}
\item Bruddstyrke
\item Frigjort metan
\item Bruddmekanisme
...
\column{0.6\textwidth}
\begin{tikzpicture}
\node at (current page.center) {\includegraphics[height=0.95\textheight]{../figures/thesis/stress_area_lefm.pdf}};
\end{tikzpicture}
\end{columns}
